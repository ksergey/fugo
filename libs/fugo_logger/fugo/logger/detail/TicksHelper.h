...
public:
  ...
  [[nodiscard]] FUGO_FORCE_INLINE auto timeSinceEpoch(std::int64_t value) noexcept -> std::int64_t {
  ...
  [[nodiscard]] FUGO_FORCE_INLINE auto nanosecondsPerTick() const noexcept -> double {
...
private:
  ...
  [[nodiscard]] FUGO_FORCE_INLINE static auto fastAvg(std::int64_t x, std::int64_t y) noexcept -> std::int64_t {
    return (x & y) + ((x ^ y) >> 1);
  }
  ...
  [[nodiscard]] FUGO_FORCE_INLINE static auto tscClockNow() noexcept -> std::int64_t {
    return __builtin_ia32_rdtsc();
  }

  [[nodiscard]] static auto calcNanosecondsPerTick() noexcept -> double {
    constexpr auto kSpinDuration = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::milliseconds(10));

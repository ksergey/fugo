...
struct BoundedSPSCRawQueueDetail {
  ...
  struct MemoryHeader {
    ...
    /// Flag indicates queue closed
    alignas(kAlign) bool closed;
  ...
  static constexpr auto alignBufferSize(std::size_t value) noexcept -> std::size_t {
    return turboq::detail::align_up(value, kSegmentSize);
  }
  ...
  static void init(std::span<std::byte> buffer) noexcept {
    ...
    std::atomic_ref{header->closed}.store(false, std::memory_order_relaxed);
  }
};
...
public:
  ...
  BoundedSPSCRawQueueProducer(turboq::MappedRegion&& storage) : storage_(std::move(storage)) {
  ...
  [[nodiscard]] FUGO_FORCE_INLINE explicit operator bool() const noexcept {
  ...
  /// Close queue
  FUGO_FORCE_INLINE void close() noexcept {
    std::atomic_ref{header_->closed}.store(true, std::memory_order_release);
  }

  /// Reserve contiguous space for writing without making it visible to the
  /// consumers. Return empty buffer on error
  [[nodiscard]] FUGO_FORCE_INLINE auto prepare(std::size_t size) noexcept -> std::span<std::byte> {
  ...
  FUGO_FORCE_INLINE void commit() noexcept {
...
public:
  ...
  BoundedSPSCRawQueueConsumer(turboq::MappedRegion&& storage) : storage_(std::move(storage)) {
    auto content = storage_.content();
  ...
  [[nodiscard]] FUGO_FORCE_INLINE auto fetch() noexcept -> std::span<std::byte const> {
    if ((consumerPosCache_ == producerPosCache_ && (producerPosCache_ = std::atomic_ref{header_->producerPos}.load(
                                                        std::memory_order_acquire)) == consumerPosCache_))
        [[unlikely]] {
      return {};
    }
  ...
  FUGO_FORCE_INLINE void consume() noexcept {
...
public:
  ...
  BoundedSPSCRawQueueImpl(
      std::string_view name, turboq::MemorySource const& memorySource = turboq::AnonymousMemorySource()) {
    auto result = memorySource.open(name, turboq::MemorySource::OpenOnly);
    if (!result) {
    ...
    if (auto storage = turboq::detail::mapFile(file_); !QueueDetail::check(storage.content())) {
  ...
  BoundedSPSCRawQueueImpl(std::string_view name, CreationOptions const& options,
      turboq::MemorySource const& memorySource = turboq::AnonymousMemorySource()) {
    auto result = memorySource.open(name, turboq::MemorySource::OpenOrCreate);
    if (!result) {
      throw std::runtime_error{"failed to open memory source"};
    }
    ...
    std::size_t const capacity = turboq::detail::align_up(options.capacityHint, pageSize);
    ...
    if (auto const fileSize = file_.getFileSize(); fileSize != 0) {
      ...
      QueueDetail::init(turboq::detail::mapFile(file_, capacity).content());
    }
  }
  ...
  [[nodiscard]] FUGO_FORCE_INLINE auto createProducer() -> Producer {
    if (!operator bool()) {
      throw std::runtime_error{"queue not initialized"};
    }
    return Producer(turboq::detail::mapFile(file_));
  }

  /// Create consumer for the queue. Throws on error.
  [[nodiscard]] FUGO_FORCE_INLINE auto createConsumer() -> Consumer {
    if (!operator bool()) {
      throw std::runtime_error{"queue not initialized"};
    }
    if (!file_.tryLock()) {
    ...
    return Consumer(turboq::detail::mapFile(file_));
  }
